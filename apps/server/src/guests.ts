  ...
  private landing(res: ServerResponse, message = "", status = 200) {
    ...
    res.writeHead(status, {
      ...
      "Content-Security-Policy":
        "default-src 'none'; img-src 'self'; script-src https://challenges.cloudflare.com; frame-src https://challenges.cloudflare.com; style-src 'unsafe-inline'; form-action 'self'; base-uri 'none'; frame-ancestors 'none'",
    });
    res.end(
      accessPage(
        `
      <span class="eyebrow">Your next coffee run</span><h2>Welcome to the outpost.</h2>
      <p class="muted">Free to play. No account needed.</p>
      ${text ? `<p class="notice" role="alert">${esc(text)}</p>` : ""}
      ${
        available
          ? `<form method="post" action="/access/guest">
        <label for="displayName">Display name <small>(optional)</small></label>
        <input id="displayName" name="displayName" maxlength="40" autocomplete="nickname" placeholder="Coffee Captain" aria-describedby="name-help">
        <small id="name-help">Skip to use a guest ID. You can change your name later. No profanity or sexual content.</small>
